...
const displayValue = computed(() => {
  ...
  if (Array.isArray(val)) {
    ...
    if (!timezoneItems.value?.length) {
      return `${val.length} timezone${val.length !== 1 ? "s" : ""} selected`;
    }
    const names = val
      .map((v) => timezoneItems.value?.find((t) => t.value === v)?.label ?? v)
      .join(", ");
    return names;
  }
  if (!timezoneItems.value?.length) return val;
  return timezoneItems.value.find((t) => t.value === val)?.label ?? val;
});
...
const {
  ...
  status,
  execute,
} = useLazyFetch("/api/geo/timezone", {
  key: "timezone-select-input",
  query: computed(() => ({
  ...
  immediate: false,
  server: false,
  transform: (items: TimezoneItem[]) => {
    return (items ?? []).map(
      (item): SelectItem => ({
        label: item.name,
        value: item.id,
        description: `${item.abbreviation} · ${item.region}`,
      }),
    );
  },
});
    ...
    >
      ...
      <template #content>
        <div class="flex w-72 flex-col">
          ...
          <!-- Empty -->
          <div
            v-else-if="!timezoneItems?.length"
          ...
          <!-- Footer count -->
          <div
            v-if="timezoneItems?.length"
            class="border-t border-default px-3 py-1.5 text-xs text-muted"
          >
            {{ timezoneItems.length }} timezone{{
              timezoneItems.length !== 1 ? "s" : ""
            }}
          </div>
        </div>
      </template>
